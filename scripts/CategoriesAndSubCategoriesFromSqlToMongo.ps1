...
$database = $server.GetDatabase("eCommerce")
...
#$sqlConnectionString = "Data Source=(local)\SQLExpress;Initial Catalog=eCommerce; Integrated Security=SSPI"
$sqlCommandText = "SELECT pc.[ProductCategoryID], pc.[Name] CategoryName,
                          ps.[ProductSubcategoryID], ps.[Name] SubcategoryName
                   FROM [eCommerce].[Production].[ProductCategory] pc 
                   INNER JOIN [eCommerce].[Production].[ProductSubcategory] ps on pc.ProductCategoryID = ps.ProductCategoryID
