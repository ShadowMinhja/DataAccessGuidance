...
function IsSupportedVersionOfPowerShell()
{
    $result = $true
    $psversion = if (test-path variable:psversiontable) {$psversiontable.psversion} else {[version]"1.0.0.0"}
    if ($psversion.Major -ne 3)
    {
        $result = $false
    }
...
function IsValidSQLServerConnectionString([string] $connectionString)
{
    $result = $false
    try
    {
    ...
    #Try to connect to make sure the server exists.
    try
    {
        $sqlConnection.Open()
        $result = $true
...
function IsValidMongoDBConnectionString([string] $connectionString)
{
    Add-Type -Path (Join-Path $scriptPath "..\.\packages\mongocsharpdriver.1.8.1\lib\net35\MongoDB.Driver.dll")
...
function IsValidNeo4jConnectionString([string] $connectionString)
{
    $result = $false
    try
    {
        Invoke-RestMethod -Uri $connectionString/db/data/ -Method Get | Out-Null
        $result = $true
    }
...
function IsValidWindowsAzureStorageConnectionString([string] $connectionString)
{
    Add-Type -Path (Join-Path $scriptPath "..\.\packages\Microsoft.Data.Edm.5.2.0\lib\net40\Microsoft.Data.Edm.dll")
    Add-Type -Path (Join-Path $scriptPath "..\.\packages\Microsoft.Data.OData.5.2.0\lib\net40\Microsoft.Data.OData.dll")
    Add-Type -Path (Join-Path $scriptPath "..\.\packages\System.Spatial.5.2.0\lib\net40\System.Spatial.dll")
    Add-Type -Path (Join-Path $scriptPath "..\.\packages\WindowsAzure.Storage.2.0.5.1\lib\net40\Microsoft.WindowsAzure.Storage.dll")

    $result = $false
    try
    {
        $azureAccount = [Microsoft.WindowsAzure.Storage.CloudStorageAccount]::Parse($connectionString)
        $result = $true
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUvTLvqc/lBR/UjaBRYsteTm6/
# 88agghWCMIIEwzCCA6ugAwIBAgITMwAAADPlJ4ajDkoqgAAAAAAAMzANBgkqhkiG
...
# HAYKKwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFCTD
# Cjejs3+XXVojwHdAfloYb4YEMGoGCisGAQQBgjcCAQwxXDBaoDKAMABwAG4AcABk
...
# AJUVjqTBMuvlrFBQW3uDerMwUOhDrr/2TbQW7BATxG8jVupjdAv6fhGDJe3UsXDe
# C9VtXzqk88tQXVpr7G32BDToyTQVO3ZmLdT1K6L0gvKUwPMLwd/AjcnqzwsSXR6z
# 74zop/lTKneEtuWUlftL153uRQzMDWgcHb3H5lhxu6JYl2pLAGB4AztXqyLcBxcZ
# 2iJMfpkE0gmQiEtW4KGxaOs4gZi7RkUPnPOiJHfpEVPVEfaNWeqXdETHgfzU0Q2H
# h85W3JLWA9kon6zozXvIviTi64VltnHtoUevzQZFSFDXzWUioJ/spWkL1fL2Ko2l
# NKnbktOzpiZ5aJd16/GHMsmhggIoMIICJAYJKoZIhvcNAQkGMYICFTCCAhECAQEw
...
# DQEJBTEPFw0xMzA4MTkyMDUzNDBaMCMGCSqGSIb3DQEJBDEWBBSYDctLu1cOidlr
# FY5HXr/O3wGhtDANBgkqhkiG9w0BAQUFAASCAQBidfxlzG/4SRRqYiQQcBlIfdhE
# khWIDlps9fm4Had79nUvkNnZOkvMSdRPpIr0jrW25yqMyN0CkAqESl4akuAQrrjL
# 7F2MGXCE2eN/k8kLxLUfuJ9AQ+m2edqpzdTS4FogtUvisnwm50NCrJoEitT48RAT
# MKiGjemfzaTYLHPSrcaK0mFjyebQmj3O7EUVLzjp8HYPTiX7UYwKcBsB3sZdwkf+
# ExdugTrA1YJMimPeD3IANErmcYQFIplJmSrm9BHFVsrgGGBYOecVYKtOWyKvVMbe
# +q0EBSP6LEGX18AFwh9+Lqz1eFb+Bv52esJt/m6y7+2uJ8nc3WHOrhMfRlp2
